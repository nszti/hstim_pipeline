...
def stim_dur_val(tiff_dir):
    ...
    for dir in filenames:
        ...
        print(f"Stimulation durations for {dir}: {stim_duration}")
        np.save(dir_path +'/stimDurations.npy', stim_duration)

#electrodeROI

def electROI_val(tiff_dir):
    base_dir = Path(tiff_dir)
    filenames = [file.name for file in base_dir.iterdir() if file.name.startswith('merged')]
    for dir in filenames:
        #print(electrodeROI)
...
#activated_neurons
def activated_neurons_val(tiff_dir):
    '''
...
#timecourse
def timecourse_vals(tiff_dir, num_trials):
    '''
    :param expDir:
...
def dist_vals (expDir):
    ...
    for dir in filenames:
        ...
        electrodeROI_path = expDir + '/' + dir + '/selected_elec_ROI.npy'
